...
//        String salt = ToolUtils.randomString(5);
    ...
    public LoginTokenVo accountLogin(LoginPwdValidate loginPwdValidate) {
        ...
        if (!config.contains(loginPwdValidate.getScene().toString())) {
        ...
        if (loginPwdValidate.getScene().equals(LoginEnum.ACCOUNT_PASSWORD.getCode())) {
            ...
            return this.__loginToken(user.getId(), user.getMobile(), user.getIsNewUser(), terminal, loginPwdValidate.getRegistrationId());
        } else {
            return mobileLogin(account, code, terminal, loginPwdValidate.getRegistrationId());
        }
    }
     ...
     */
    @Override
    public LoginTokenVo mobileLogin(String mobile, String code, Integer terminal, String registrationId) {
        // 校验验证码
...
//        Assert.notNull(user, "账号不存在!");
     ...
     * @return LoginTokenVo
     */
    private LoginTokenVo __loginToken(Integer userId, String mobile, Integer isNew, Integer terminal, String registrationId) {
        // 实现账号登录
