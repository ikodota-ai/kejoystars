...
public class FinanceRechargerController {
    ...
    @PostMapping("/add")
    @ApiModelProperty("手动充值")
    public AjaxResult<Object> add(@Validated @RequestBody FinanceAddValidate financeAddValidate) {
        Integer adminId = LikeAdminThreadLocal.getAdminId();

        iFinanceRechargerService.add(financeAddValidate);
        return AjaxResult.success("【"+ financeAddValidate.getAccount() +"】充值成功！", true);
    }
