...
import com.mdd.admin.validate.finance.FinanceAddValidate;
...
public class FinanceRechargerServiceImpl implements IFinanceRechargerService {
    ...
    @Override
    @Transactional
    public void add(FinanceAddValidate financeAddValidate) {
        //检查账号是否存在
        User user = userMapper.selectOne(new QueryWrapper<User>().eq("account", financeAddValidate.getAccount()));
        Assert.notNull(user, "账号不存在!");
        long timestamp = System.currentTimeMillis()/1000;
        //添加充值记录
        RechargeOrder rechargeOrder = new RechargeOrder();
        rechargeOrder.setUserId(user.getId());
        rechargeOrder.setRid(0);
        rechargeOrder.setOrderAmount(financeAddValidate.getAmount());
        rechargeOrder.setPayWay(2);
        rechargeOrder.setPayStatus(1);
        rechargeOrder.setSn(ToolUtils.makeUUID());
        rechargeOrder.setPayTime( timestamp );
        rechargeOrder.setCreateTime( timestamp );
        rechargeOrder.setUpdateTime( timestamp );

        rechargeOrderMapper.insert(rechargeOrder);
        //修改用户过期时间
        if (user.getVipExpired() <= timestamp) {
            user.setVipExpired( (int)timestamp + 60 * 60 * 24 * rechargeOrder.getOrderAmount().intValue() );
            user.setVipDownCount( 50 );
        } else {
            user.setVipExpired(user.getVipExpired() + 60 * 60 * 24 * rechargeOrder.getOrderAmount().intValue() );
            if (user.getVipDownCount() <= 0 || user.getVipDownCount() > 50) {
                user.setVipDownCount( 50 );
            }
        }
