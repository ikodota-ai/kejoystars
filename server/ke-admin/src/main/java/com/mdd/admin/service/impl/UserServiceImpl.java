...
public class UserServiceImpl implements IUserService {
    ...
    public PageResult<UserVo> list(PageValidate pageValidate, UserSearchValidate searchValidate) {
        ...
        for (User user : iPage.getRecords()) {
            ...
            vo.setVipExpired(user.getVipExpired() > (int)(System.currentTimeMillis()/1000) ?
                    (user.getVipExpired() -(int) (System.currentTimeMillis()/1000))/oneDay : 0);

            //取得总签到次数
            Integer signCount = Math.toIntExact(userCheckInMapper.selectCount(new QueryWrapper<UserCheckIn>().eq("userid", user.getId())));

            vo.setSignCount(signCount + "/" + (((System.currentTimeMillis()/1000) - user.getCreateTime())/oneDay));
            //取得当前用户邀请人数
            int inviteCount = Math.toIntExact(userMapper.selectCount(new QueryWrapper<User>().eq("parent_invitation_code", user.getInvitationCode())));
            vo.setInviteCount((Math.min(inviteCount, 3)) + " / " + inviteCount);
